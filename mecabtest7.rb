...
h.each {|text|
  ...
  # puts adjective_count.keys.join("\t") + "\t" + noun_count.keys.join("\t")  # 単語全部のキーと個数をタブ区切りで出力
  # puts adjective_count.select {|k, v| v >= 5 }.keys.join("\t") + "\t" + noun_count.select {|k, v| v >= 5 }.keys.join("\t") # n個以上ある単語のキーだけ取り出してタブ区切りで出力
